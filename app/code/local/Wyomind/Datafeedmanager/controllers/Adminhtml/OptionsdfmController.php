 ...
 */
class Wyomind_Datafeedmanager_Adminhtml_OptionsdfmController extends Mage_Adminhtml_Controller_Action
{
        ...
        if ($this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('datafeedmanager')->__('The custom option has been saved.')
                );
    ...
    {  
        if ($this->getRequest()->getParam('id')) {
            ...
            try {
                ...
                $model->delete();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('datafeedmanager')->__('The custom option has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Exception $e) {
        ...
        // display error message
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('datafeedmanager')->__('Unable to find the custom option to delete.')
        );
        // go to grid
        $this->_redirect('*/*/');
    }
