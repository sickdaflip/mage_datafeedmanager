...
class Wyomind_Datafeedmanager_Block_Adminhtml_Configurations_Edit_Tab_Ftp extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset->addField(
            'ftp_ssl', 'select', array(
            'label' => Mage::helper('datafeedmanager')->__('Use SSL'),
            'name' => 'ftp_ssl',
            'id' => 'ftp_ssl',
            'required' => true,
            'values' => array(
        ...
        $fieldset->addField(
            ...
            'name' => 'ftp_dir',
            'id' => 'ftp_dir',
            'note' => "<a style='margin:10px; display:block;' href='javascript:datafeedmanager.testFtp(\"" 
                    . $this->getUrl('*/*/ftp') . "\")'>Test Connection</a>"
            )
        );
        ...
        if (version_compare(Mage::getVersion(), '1.3.0', '>')) {
            $this->setChild(
                'form_after', $this->getLayout()->createBlock('adminhtml/widget_form_element_dependence')
                            ...
                            ->addFieldDependence('ftp_ssl', 'use_sftp', 0)
                            ->addFieldDependence('ftp_ssl', 'ftp_enabled', 1)
